...
# --- 5. Routing Logic (Regular vs Admin via Secret URL) ---
query_params = st.query_params
is_admin_url = query_params.get("mode") == "admin"

# Session State
if "history" not in st.session_state:
    st.session_state.history = [{"role": "bot", "text": "আসসালামু আলাইকুম। আমি **BRAC IED** থেকে বলছি। আপনার মানসিক স্বাস্থ্য মূল্যায়নে আপনাকে স্বাগতম।"}]
...
if is_admin_url:
    if not st.session_state.admin_logged_in:
        ...
        if not df.empty:
            ...
            st.info("No participants have completed the assessment yet.")

# --- 7. USER ASSESSMENT ROUTE ---
else:
    # Display Chat History
    for msg in st.session_state.history:
        if msg["role"] == "bot":
            st.markdown(f'<div class="message-row"><div class="avatar bot-avatar">B</div><div class="content-area"><div class="bot-name">BRAC IED</div><div class="bot-text">{msg["text"]}</div></div></div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="message-row"><div class="avatar user-avatar">YOU</div><div class="content-area user-text">{msg["text"]}</div></div>', unsafe_allow_html=True)

    if not st.session_state.done:
        step = st.session_state.step
        if step < len(questions):
            # Show Instructional Labels
            if step == 0:
    ...
    # Final Results View
        ...
        st.markdown(f"""
            <div class="result-container" style="background-color: {bg}; border-color: {color};">
                ...
                <div style="background-color: {color}; color: white; display: inline-block; padding: 10px 35px; border-radius: 100px; font-weight: 600; font-size: 1.4rem; margin: 25px 0;">
                    {lvl}
                </div>
